...
__device__ __inline__ void trace_ray_w_alpha_depth_cuvol(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t lane_id,
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            const float weight = _EXP(log_transmit) * (1.f - _EXP(-pcnt));
            alpha += weight;
            depth += weight * (t / opt.step_size) * ray.world_step;
            log_transmit -= pcnt;

            float lane_color_total = WarpReducef(temp_storage).HeadSegmentedSum(
                                           lane_color, lane_colorgrp_id == 0);
            lane_color_total = fmaxf(lane_color_total + 0.5f, 0.f);  // Clamp to [+0, infty)
            outv += weight * lane_color_total;
            if (_EXP(log_transmit) < opt.stop_thresh) {
                log_transmit = -1e3f;
                break;
            }
    ...
    if (lane_colorgrp_id == 0) {
        *out_alpha = alpha;
        *out_depth = depth / max(alpha, 1e-10);
        if (out_log_transmit != nullptr) {
            *out_log_transmit = log_transmit;
        }
        out[lane_colorgrp] = outv;
...
__device__ __inline__ void trace_ray_vardepth_cuvol(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t lane_id,
        ...
        WarpReducef::TempStorage& __restrict__ temp_storage,
        float* __restrict__ out_vardepth,
        float* __restrict__ out_sqrtvardepth) {
    const uint32_t lane_colorgrp_id = lane_id % grid.basis_dim;
    const uint32_t lane_colorgrp = lane_id / grid.basis_dim;

    if (ray.tmin > ray.tmax) {
        *out_vardepth = 0.f;
        *out_sqrtvardepth = 0.f;
        return;
    }
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            const float weight = _EXP(log_transmit) * (1.f - _EXP(-pcnt));
            alpha += weight;
            vardepth += weight * (expected_depth[0] - t / opt.step_size * ray.world_step) * (expected_depth[0] - t / opt.step_size * ray.world_step);
            sqrtvardepth += weight * (expected_depth[0] - t / opt.step_size * ray.world_step);
            log_transmit -= pcnt;
    ...
    if (lane_colorgrp_id == 0) {
        *out_vardepth = vardepth / max(alpha, 1e-10);
        *out_sqrtvardepth = sqrtvardepth / max(alpha, 1e-10);
    }
}
...
__device__ __inline__ void trace_ray_w_alpha_depth_cuvol_backward(
        const PackedSparseGridSpec& __restrict__ grid,
        const float* __restrict__ grad_output,
        const float* __restrict__ grad_output_alpha,
        const float* __restrict__ grad_output_depth,
        const float* __restrict__ grad_output_vardepth,
        const float* __restrict__ color_cache,
        const float* __restrict__ alpha_cache,
        const float* __restrict__ depth_cache,
        const float* __restrict__ vardepth_cache,
        const float* __restrict__ sqrtvardepth_cache,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t lane_id,
        const float* __restrict__ sphfunc_val,
    ...
    const uint32_t lane_colorgrp_id = lane_id % grid.basis_dim;
    const uint32_t lane_colorgrp = lane_id / grid.basis_dim;
    const uint32_t cout = grid.sh_data_dim / grid.basis_dim;

    uint32_t leader_mask = 1U;
    for (int j = 1; j < cout; ++j) {
        leader_mask = leader_mask | (1U << (j * grid.basis_dim));
    }

    float accum = 0.f;
    for (int j = (cout-1); j >= 0; --j) {                   // perform loop backwards to keep same precision as before
        accum = fmaf(color_cache[j], grad_output[j], accum);
    }
    float accum_alpha = alpha_cache[0] * grad_output_alpha[0];
    float accum_depth = depth_cache[0] * grad_output_depth[0];
    float accum_vardepth = vardepth_cache[0] * grad_output_vardepth[0];
    float accum_depth_vardepthgrad = depth_cache[0] * grad_output_vardepth[0];
    float accum_alpha_depthgrad = alpha_cache[0] * grad_output_depth[0];
    float accum_alpha_vardepthgrad = alpha_cache[0] * grad_output_vardepth[0];
    ...
    const float gout = grad_output[lane_colorgrp];
    const float gout_alpha = grad_output_alpha[0];
    const float gout_depth = grad_output_depth[0];
    const float gout_vardepth = grad_output_vardepth[0];
    const float expected_depth = depth_cache[0];
    const float grad_from_depthnorm = -1/alpha_cache[0] * depth_cache[0];
    const float grad_from_vardepthnorm = -1/alpha_cache[0] * vardepth_cache[0];
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            float total_depth = (t / opt.step_size) * ray.world_step / max(alpha_cache[0], 1e-10);
            total_depth *= gout_depth;

            float total_depth_vardepthgrad = (t / opt.step_size) * ray.world_step / max(alpha_cache[0], 1e-10);
            total_depth_vardepthgrad *= gout_vardepth;

            float total_vardepth = (expected_depth - t / opt.step_size * ray.world_step)*(expected_depth - t / opt.step_size * ray.world_step) / max(alpha_cache[0], 1e-10);
            total_vardepth *= gout_vardepth;

            float lane_color_total = WarpReducef(temp_storage).HeadSegmentedSum(
                                           weighted_lane_color, lane_colorgrp_id == 0);
            float total_color = lane_color_total;
            lane_color_total += 0.5f;
            total_color = fmaxf(lane_color_total, 0.f);
            float color_in_01 = total_color == lane_color_total;
            total_color *= gout; // Clamp to [+0, infty)

            float total_color_c1 = 0.f;
            for (int j = 1; j < (cout-1); ++j) {
                total_color_c1 += __shfl_sync(leader_mask, total_color, j * grid.basis_dim);
            }
            total_color += __shfl_sync(leader_mask, total_color, (cout-1) * grid.basis_dim);
            total_color += total_color_c1;
            ...
            accum -= weight * total_color;
            accum_alpha -= weight * gout_alpha;
            accum_depth -= weight * total_depth;
            accum_vardepth -= weight * total_vardepth;
            accum_depth_vardepthgrad -= weight * total_depth_vardepthgrad;
            float curr_grad_sigma_from_depth_vardepthgrad = ray.world_step * (
                    total_depth_vardepthgrad * _EXP(log_transmit) - accum_depth_vardepthgrad);
            float curr_grad_sigma = ray.world_step * (
                    total_color * _EXP(log_transmit) - accum);
            curr_grad_sigma += ray.world_step * (
                    gout_alpha * _EXP(log_transmit) - accum_alpha);
            if (alpha_cache[0] > 1e-10) {             // losses from normalization
                accum_alpha_depthgrad -= weight * gout_depth;
                accum_alpha_vardepthgrad -= weight * gout_vardepth;
                float curr_grad_sigma_from_alpha_depthgrad = ray.world_step * (gout_depth * _EXP(log_transmit) - accum_alpha_depthgrad);
                float curr_grad_sigma_from_alpha_vardepthgrad = ray.world_step * (gout_vardepth * _EXP(log_transmit) - accum_alpha_vardepthgrad);
                curr_grad_sigma += grad_from_depthnorm * curr_grad_sigma_from_alpha_depthgrad;
                curr_grad_sigma += grad_from_vardepthnorm * curr_grad_sigma_from_alpha_vardepthgrad;
                curr_grad_sigma_from_depth_vardepthgrad += grad_from_vardepthnorm * curr_grad_sigma_from_alpha_vardepthgrad;
            }
            curr_grad_sigma += ray.world_step * (
                    total_depth * _EXP(log_transmit) - accum_depth);
            curr_grad_sigma +=
                    ray.world_step * (total_vardepth * _EXP(log_transmit) - accum_vardepth) +
                    2 * sqrtvardepth_cache[0] * curr_grad_sigma_from_depth_vardepthgrad;
            if (sparsity_loss > 0.f) {
                // Cauchy version (from SNeRG)
                curr_grad_sigma += sparsity_loss * (4 * sigma / (1 + 2 * (sigma * sigma)));
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_w_alpha_depth_kernel(
        PackedSparseGridSpec grid,
        PackedRaysSpec rays,
        RenderOptions opt,
        torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> out,
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_vardepth_kernel(
        PackedSparseGridSpec grid,
        PackedRaysSpec rays,
        RenderOptions opt,
        const float* __restrict__ expected_depth,
    ...
    trace_ray_vardepth_cuvol(
        grid,
        ray_spec[ray_blk_id],
        opt,
        lane_id,
        expected_depth + ray_id,
        temp_storage[ray_blk_id],
        out_vardepth + ray_id,
        out_sqrtvardepth + ray_id);
}
...
__launch_bounds__(TRACE_RAY_BKWD_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_w_alpha_depth_backward_kernel(
    PackedSparseGridSpec grid,
    const float* __restrict__ grad_output,
    const float* __restrict__ grad_output_alpha,
    const float* __restrict__ grad_output_depth,
    ...
    if (grad_out_is_rgb) {
        assert(0);  // not implemented with feature rendering
    }
    const uint32_t cout = grid.sh_data_dim / grid.basis_dim;

    __syncwarp((1U << grid.sh_data_dim) - 1);
    trace_ray_w_alpha_depth_cuvol_backward(
        grid,
        grad_output + ray_id * cout,
        grad_output_alpha + ray_id,
        grad_output_depth + ray_id,
        grad_output_vardepth + ray_id,
        color_cache + ray_id * cout,
        alpha_cache + ray_id,
        depth_cache + ray_id,
        vardepth_cache + ray_id,
        sqrtvardepth_cache + ray_id,
        ray_spec[ray_blk_id],
        opt,
        lane_id,
        sphfunc_val[ray_blk_id],
...
torch::Tensor _get_empty_Nd(const torch::Tensor& origins, const int N) {
    auto options =
        torch::TensorOptions()
        .dtype(origins.dtype())
        .layout(torch::kStrided)
        .device(origins.device())
        .requires_grad(false);
    return torch::empty({origins.size(0), N}, options);
}
...
torch::Tensor volume_render_w_alpha_depth_cuvol(SparseGridSpec& grid, RaysSpec& rays, RenderOptions& opt, torch::Tensor alpha_out, torch::Tensor depth_out, torch::Tensor vardepth_out, torch::Tensor sqrtvardepth_out, bool render_vardepth) {
    DEVICE_GUARD(grid.sh_data);
    CHECK_INPUT(alpha_out);
    CHECK_INPUT(depth_out);
    CHECK_INPUT(vardepth_out);
    CHECK_INPUT(sqrtvardepth_out);
    grid.check();
    rays.check();
    ...
    const int cout = grid.sh_data.size(1) / grid.basis_dim;
    torch::Tensor results = _get_empty_Nd(rays.origins, cout);
...

    {
        const int cuda_n_threads = TRACE_RAY_CUDA_THREADS;
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q * WARP_SIZE, cuda_n_threads);
        device::render_ray_w_alpha_depth_kernel<<<blocks, cuda_n_threads>>>(
                grid, rays, opt,
                // Output
                results.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
    ...
    if (render_vardepth) {
        const int cuda_n_threads = TRACE_RAY_CUDA_THREADS;
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q * WARP_SIZE, cuda_n_threads);
        device::render_ray_vardepth_kernel<<<blocks, cuda_n_threads>>>(
                grid, rays, opt,
                depth_out.data_ptr<float>(),
                // Output
                vardepth_out.data_ptr<float>(),
                sqrtvardepth_out.data_ptr<float>());
    }
...
void volume_render_w_alpha_depth_cuvol_backward(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        torch::Tensor grad_out,
        ...
        torch::Tensor grad_out_vardepth,
        torch::Tensor color_cache,
        torch::Tensor alpha_cache,
        torch::Tensor depth_cache,
        torch::Tensor vardepth_cache,
        torch::Tensor sqrtvardepth_cache,
        GridOutputGrads& grads) {
...

    {
        const int cuda_n_threads_render_backward = TRACE_RAY_BKWD_CUDA_THREADS;
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q * WARP_SIZE, cuda_n_threads_render_backward);
        device::render_ray_w_alpha_depth_backward_kernel<<<blocks,
            cuda_n_threads_render_backward>>>(
                    grid,
                    grad_out.data_ptr<float>(),
                    grad_out_alpha.data_ptr<float>(),
                    grad_out_depth.data_ptr<float>(),
                    grad_out_vardepth.data_ptr<float>(),
                    color_cache.data_ptr<float>(),
                    alpha_cache.data_ptr<float>(),
                    depth_cache.data_ptr<float>(),
                    vardepth_cache.data_ptr<float>(),
                    sqrtvardepth_cache.data_ptr<float>(),
                    rays, opt,
                    false,
                    nullptr,
                    0.f,
...
torch::Tensor volume_render_reg_cuvol(SparseGridSpec& grid, RaysSpec& rays, RenderOptions& opt, float beta_loss, torch::Tensor log_transmit_out) {
    DEVICE_GUARD(grid.sh_data);
    CHECK_INPUT(log_transmit_out);
    grid.check();
    rays.check();
...

    {
        ...
        device::render_ray_kernel<<<blocks, TRACE_RAY_CUDA_THREADS>>>(
                ...
                // Output
                results.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                need_log_transmit ? log_transmit_out.data_ptr<float>() : nullptr);
    }
    ...
    if (use_background) {
        ...
        device::render_background_kernel<<<blocks, TRACE_RAY_BG_CUDA_THREADS>>>(
                ...
                rays,
                opt,
                log_transmit_out.data_ptr<float>(),
                results.packed_accessor32<float, 2, torch::RestrictPtrTraits>());
    }
...
void volume_render_reg_cuvol_backward(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        float beta_loss,
        ...
        torch::Tensor grad_out,
        torch::Tensor color_cache,
        torch::Tensor log_transmit_cache,
        GridOutputGrads& grads)  {

    DEVICE_GUARD(grid.sh_data);
    CHECK_INPUT(grad_out);
    CHECK_INPUT(color_cache);
    CHECK_INPUT(log_transmit_cache);
    grid.check();
    rays.check();
    grads.check();
    const auto Q = rays.origins.size(0);
...

    {
        ...
        device::render_ray_backward_kernel<<<blocks,
            TRACE_RAY_BKWD_CUDA_THREADS>>>(
                    ...
                    color_cache.data_ptr<float>(),
                    rays, opt,
                    false,      // Do not compute MSE loss
                    beta_loss > 0.f ? log_transmit_cache.data_ptr<float>() : nullptr,
                    beta_loss / Q,
                    sparsity_loss,
                    // Output
                    grads,
                    use_background ? accum.data_ptr<float>() : nullptr,
                    use_background ? log_transmit_cache.data_ptr<float>() : nullptr);
    }
    ...
    if (use_background) {
        ...
        device::render_background_backward_kernel<<<blocks, TRACE_RAY_BG_CUDA_THREADS>>>(
                ...
                rays,
                opt,
                log_transmit_cache.data_ptr<float>(),
                accum.data_ptr<float>(),
                false,      // Do not compute MSE loss
                sparsity_loss,
                // Output
