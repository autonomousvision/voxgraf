...
class _VolumeRenderFunction(autograd.Function):
    ...
    def forward(
        ...
        render_alpha: bool,
        render_depth: bool,
        render_vardepth: bool,
    ):
        alpha = None
        depth = None
        vardepth = None
        sqrtvardepth = None
        use_adapted_kernel = render_alpha or render_depth or render_vardepth
        if use_adapted_kernel:
            alpha = torch.zeros_like(rays.origins[:, 0])
            depth = torch.zeros_like(rays.origins[:, 0])
            vardepth = torch.zeros_like(rays.origins[:, 0])
            sqrtvardepth = torch.zeros_like(rays.origins[:, 0])
            cu_fn = _C.__dict__[f"volume_render_w_alpha_depth_{backend}"]
            color = cu_fn(grid, rays, opt, alpha, depth, vardepth, sqrtvardepth, render_vardepth)
            if not render_vardepth:
                assert (vardepth == 0).all()
                assert (sqrtvardepth == 0).all()
        else:
            cu_fn = _C.__dict__[f"volume_render_{backend}"]
            color = cu_fn(grid, rays, opt)
        ctx.save_for_backward(color, alpha, depth, vardepth, sqrtvardepth)
        ctx.grid = grid
        ctx.rays = rays
        ctx.opt = opt
        ctx.backend = backend
        ctx.basis_data = data_basis
        ctx.use_adapted_kernel = use_adapted_kernel
        return color, alpha, depth, vardepth

    @staticmethod
    def backward(ctx, grad_out, grad_out_alpha, grad_out_depth, grad_out_vardepth):
        (color_cache, alpha_cache, depth_cache, vardepth_cache, sqrtvardepth_cache) = ctx.saved_tensors
        if ctx.use_adapted_kernel:
            cu_fn = _C.__dict__[f"volume_render_w_alpha_depth_{ctx.backend}_backward"]
        else:
        ...
        if ctx.use_adapted_kernel:
            cu_fn(
                ...
                ctx.opt,
                grad_out.contiguous(),
                grad_out_alpha.contiguous(),
                grad_out_depth.contiguous(),
                grad_out_vardepth.contiguous(),
                color_cache,
                alpha_cache,
                depth_cache,
                vardepth_cache,
...
class SparseGrid(nn.Module):
    ...
    def _volume_render_gradcheck_lerp(self, rays: Rays, return_raylen: bool=False, return_alpha: bool=False, return_depth: bool=False, return_vardepth: bool=False):
        """
        trilerp gradcheck version
        """
        if return_vardepth:      # render everything to get expected depth
            expected_depth = self._volume_render_gradcheck_lerp(rays=rays, return_raylen=return_raylen, return_alpha=False, return_depth=True, return_vardepth=False)[:, -1]
        ...
        cout = (self.data_dim-1) // self.basis_dim
        if self.basis_type == BASIS_TYPE_3D_TEXTURE:
        ...
        if return_alpha:
            out_rgb = torch.cat([out_rgb, out_alpha], 1)
        if return_depth:
            out_rgb = torch.cat([out_rgb, out_depth], 1)
        if return_vardepth:
            out_rgb = torch.cat([out_rgb, out_vardepth], 1)
        return out_rgb
    ...
    def volume_render(
        ...
        return_raylen: bool=False, render_alpha: bool=False, render_depth: bool=False, render_vardepth: bool=False,
    ):
        """
        ...
        if use_kernel and self.links.is_cuda and _C is not None and not return_raylen:
            ...
            rgb_out, alpha_out, depth_out, vardepth_out = _VolumeRenderFunction.apply(
                self.density_data,
                self.sh_data,
                basis_data,
            ...
            if render_alpha:
                rgb_out = torch.cat([rgb_out, alpha_out[:, None]], 1)
            if render_depth:
                rgb_out = torch.cat([rgb_out, depth_out[:, None]], 1)
            if render_vardepth:
                rgb_out = torch.cat([rgb_out, vardepth_out[:, None]], 1)
            return rgb_out
        else:
            ...
            if self.opt.backend == 'nvol':
                ...
                rgb = self._volume_render_gradcheck_lerp(rays, return_raylen=return_raylen, return_alpha=render_alpha, return_depth=render_depth, return_vardepth=render_vardepth)
                return rgb
    ...
    def volume_render_fused(
        ...
        if render_alpha:
            assert self.opt.backend == 'cuvol'
            cu_fn = _C.__dict__[f"volume_render_logt_{self.opt.backend}"]
            logT = cu_fn(
                self._to_cpp(replace_basis_data=basis_data),
                rays._to_cpp(),
                self.opt._to_cpp(randomize=randomize))
            rgb_out = torch.cat([rgb_out, logT.exp().unsqueeze(-1)], -1)
