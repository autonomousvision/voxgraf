...
with Timing("ours"):
    samps = grid.volume_render(rays, use_kernel=True, render_alpha=dims==5, render_depth=True)[:, -dims:]
    # samps_original = grid.volume_render_depth(rays)[:, None]#, use_kernel=True, render_alpha=False, render_depth=True)
# assert (samps == samps_original).all(), 'Depth implementation does not match original plenoxel implementation'            # original implementation does not feature background depth
...
if ENABLE_TORCH_CHECK:
    with Timing("torch"):
        sampt = grid.volume_render(rays, use_kernel=False, render_alpha=dims==5, render_depth=True)[:, -dims:]
