...
feat_dim = 3*grid.basis_dim
grid.sh_data.data = torch.empty([grid.sh_data.shape[0], feat_dim], device=grid.sh_data.device, dtype=grid.sh_data.dtype)
...
from training.virtual_camera_utils import RaySampler, PoseSampler
img_res = 32
ps = PoseSampler(range_azim=(180, 45), range_polar=(90, 15), radius=8, dist='normal')
rs = RaySampler(img_resolution=img_res, fov=30)
pose = ps.sample_from_dist()
origins, dirs = rs.sample_at(pose[None, :3, :4], rs.img_resolution, fov=30)
...
dims = feat_dim + 3
rgb_gt = torch.zeros((origins.size(0), dims), device=device, dtype=dtype)
...
with Timing("ours"):
    samps = grid.volume_render(rays, use_kernel=True, render_alpha=dims==(feat_dim + 3), render_depth=True, render_vardepth=True)[:, -dims:]
...
if ENABLE_TORCH_CHECK:
    with Timing("torch"):
        sampt = grid.volume_render(rays, use_kernel=False, render_alpha=dims==(feat_dim + 3), render_depth=True, render_vardepth=True)[:, -dims:]
    ...
    grid_density_grad_t = grid_density_grad_t.view(grid_res, grid_res, grid_res)
    grid_density_grad_s = grid_density_grad_s.view(grid_res, grid_res, grid_res)
